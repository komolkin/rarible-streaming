...
export function LivepeerProvider({ children }: { children: ReactNode }) {
  ...
  if (!apiKey) {
    console.warn(
      "⚠️ NEXT_PUBLIC_LIVEPEER_API_KEY is not set. Player may have limited functionality."
    );
    console.warn(
      "💡 Add NEXT_PUBLIC_LIVEPEER_API_KEY to .env.local with the same value as LIVEPEER_API_KEY"
    );
  } else {
